 ...
 */
static inline void vchan_complete(struct tasklet_struct *t) {
...
static inline struct dma_chan *dma_request_chan(struct drvmgr_dev *dev, const char *name) {
	struct dma_device *dma = (struct dma_device *)dev->priv;
	int i = 0;
	int channel = -EINVAL;
    _Assert(dev != NULL);
...
static inline void dma_release_chan(struct dma_chan *chan) {
	struct dma_device *dma = (struct dma_device *)dev->priv;
	if (channel < dma->dma_channels) {
		atomic_clear_bit(dma->channel_bitmap, channel);
		if (device->device_free_chan_resources)
