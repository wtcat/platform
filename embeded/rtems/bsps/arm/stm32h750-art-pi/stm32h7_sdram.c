...
static void __notrace SDRAM_Initialization_Sequence(SDRAM_HandleTypeDef *hsdram, 
...
static int __notrace SDRAM_Init(void) {
    int result = 0;
    ...
    if (HAL_SDRAM_Init(&hsdram1, &SDRAM_Timing) != HAL_OK) {
        printk("SDRAM init failed!");
        result = -EIO;
    } else {
        /* Program the SDRAM external device */
        SDRAM_Initialization_Sequence(&hsdram1, &command);
        printk("sdram init success, mapped at 0x%X, size is %d bytes, data width is %d", 
...
void __notrace stm32h7_sdram_init(void) {
