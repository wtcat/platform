...
static void stm32_dma_dump_stream_irq(struct drvmgr_dev *dev, uint32_t id) {
	struct dma_private *priv = dev->priv;
	DMA_TypeDef *dma = (DMA_TypeDef *)priv->base;
...
static const struct dma_driver_api stm32_dma_ops = {
...
static int stm32_dma_probe(struct drvmgr_dev *dev) {
	...
	priv->base = devp->base;
	priv->support_m2m = !!devcie_get_property(dev, "st,mem2mem");
	for (uint32_t i = 0; i < priv->max_streams; i++) {
		err = drvmgr_interrupt_register(dev, IRQF_HARD_REL(i), 
			dev->name, isrs[i], dev);
		if (err)
			goto _failed;
...
static struct drvmgr_drv_ops stm32_dma_driver_ops = {
	.init = {
		stm32_dma_probe,
	},
};
