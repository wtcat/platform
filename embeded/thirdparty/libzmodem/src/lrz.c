...
static const char *_zm_program_name;		/* the name by which we were called */
...
rz_receive(rz_t *rz)
{
	...
	log_info(_("%s waiting to receive."), _zm_program_name);
	c = 0;
...
fubar:
	...
	if (rz->restricted && rz->pathname) {
		unlink(rz->pathname);
		log_info(_("%s: %s removed."), _zm_program_name, rz->pathname);
	}
	return ERROR;
}
...
rz_process_header(rz_t *rz, char *name, struct zm_fileinfo *zi)
{
	...
	if (rz->topipe > 0) {
		...
		sprintf(rz->pathname, "%s %s", _zm_program_name+2, name_static);
		log_info("%s: %s %s",
			 _("Topipe"),
	...
	if (rz->restricted) {
		...
		if ( strstr(name, "../")
		 ...
		 || (name[0]== '/' && strncmp(name, PUBDIR,
			...
			zreadline_canit(rz->zm->zr, STDOUT_FILENO);
			log_info(_("%s: Security Violation"),_zm_program_name);
			bibi(-1);
		}
		if (rz->restricted > 1) {
