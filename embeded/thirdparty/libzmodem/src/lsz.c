...
usage(int exitcode, const char *what)
{
	...
	{
		if (what)
			log_info("%s: %s",_zm_program_name,what);
		log_info (_("Try `%s --help' for more information."), _zm_program_name);
		exit(exitcode);
	}

	display(_("%s version %s"), _zm_program_name, VERSION);

	display(_("Usage: %s [options] file ..."), _zm_program_name);
